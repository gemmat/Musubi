...
function main() {
  ...
  if (!Musubi.location.info) {
    Musubi.location.info = {
      auth: "satou@gmail.com",
      path: null,
      query: null,
      frag: null
    };
  ...
  var p = Musubi.parseJID(Musubi.location.info.auth);
  ...
  var string0 = "xmpp://" + Musubi.location.info.auth + "/" + gemma + "#" + chat;
  insert0.setAttribute("href", string0);
  insert0.appendChild(document.createTextNode(string0));
  insert1.appendChild(document.createTextNode(Musubi.location.info.auth));
